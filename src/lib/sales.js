...
export const getTodayDeals = (now = Date.now()) => {
  ...
  // 日曜日の「プレミアムな日曜日」はLYPプレミアム（月額508円）会員限定で、
  // さらに合計5,000円以上の決済が条件。誰にでも当てはまるように
  // 「本日プレミアムな日曜日」とだけ出すと、会員でない大多数には
  // 得られない特典を提示することになるため、条件を明示する。
  else if (dow === 0) deals.push({ shop: 'yahoo', label: '日曜はLYP会員+5%' });
  return deals;
};
...
export const SALE_CALENDAR = [
  {
    ...
    events: [
      ...
      { name: '5と0のつく日', timing: '毎月5・10・15・20・25・30日', tip: '楽天カード払いで+1倍（SPU分と合わせて計4倍）。要エントリー。急ぎの買い物はこの日に合わせる' },
      { name: 'ご愛顧感謝デー', timing: '毎月18日', tip: '会員ランクがゴールド以上だとポイントアップ。ランクによって倍率が変わる' },
    ],
  },
  {
    shop: 'Yahoo!ショッピング',
    events: [
      { name: '5のつく日', timing: '毎月5・15・25日', tip: '+4%のPayPayポイント。要エントリー＋PayPay系の決済が条件。付与上限あり' },
      { name: 'ファーストデイ', timing: '毎月1日', tip: '+3%のPayPayポイント（1注文3,000円以上）。大型キャンペーンと重なる月は実施されないこともある' },
      { name: 'ヤフショ感謝デー', timing: '毎月11・22日', tip: '旧「ゾロ目の日」。会員ランクがシルバーで+4%・ゴールドで+5%。5のつく日を逃したときの受け皿に' },
      { name: 'プレミアムな日曜日', timing: '毎週日曜', tip: 'LYPプレミアム（月額508円）会員が5,000円以上買うと+5%。会員でなければ対象外なので注意' },
