...
export const CATEGORY_TREE = [
  ...
  { name: "ベビーカー", id: "200833", keyword: "ベビーカー", subs: ["A型", "B型", "AB型", "バギー", "周辺グッズ", "その他"] },
  { name: "抱っこ紐", id: "412209", keyword: "抱っこ紐", subs: ["縦抱き", "横抱き", "スリング", "ヒップシート", "周辺グッズ", "その他"] },
  { name: "ウェア", id: "111102", keyword: "ベビー服", subs: ["ロンパース", "カバーオール", "肌着", "アウター", "スタイ", "その他"] },
  { name: "ミルク・授乳", id: "205208", keyword: "ミルク 授乳", subs: ["ミルク", "哺乳瓶", "搾乳器", "授乳クッション", "母乳パッド", "その他"] },
  { name: "離乳食・食器", id: "213980", keyword: "離乳食", subs: ["ベビーフード", "食器セット", "ベビーチェア", "スプーン", "その他"] },
  { name: "寝具・ベッド", id: "200822", keyword: "ベビーベッド", subs: ["ベビーベッド", "ベビー布団", "スリーパー", "まくら", "その他"] },
  { name: "おもちゃ", id: "201591", keyword: "おもちゃ", subs: ["0ヶ月〜", "3ヶ月〜", "6ヶ月〜", "1歳〜", "その他"] },
  // 絵本: 楽天では「本・雑誌・コミック」ジャンル配下のため、ベビー用品ジャンル(100533)
  // 直下にIDを持たせず、レンタルと同様にキーワードマッチで関連性を担保する
  { name: "絵本", id: null, keyword: "絵本 赤ちゃん", subs: ["0歳〜", "1歳〜", "2歳〜", "しかけ絵本", "その他"] },
  { name: "安全グッズ", id: "200841", keyword: "ベビーゲート", subs: ["ベビーゲート", "コーナーガード", "扉ロック", "転倒防止", "ベビーモニター", "その他"] },
  { name: "お風呂用品", id: "200815", keyword: "ベビー お風呂", subs: ["ベビーバス", "ベビー用ソープ", "保湿クリーム", "その他"] },
  { name: "トイレ用品", id: "200819", keyword: "おまる", subs: ["補助便座", "おまる", "トイトレ", "おしりふき", "その他"] },
  { name: "車用品", id: "566088", keyword: "チャイルドシート", subs: ["新生児用", "1歳以上", "ジュニアシート", "2wayタイプ", "周辺グッズ", "その他"] },
  { name: "マタニティ", id: "553946", keyword: "マタニティ", subs: ["マタニティウェア", "腹帯", "葉酸サプリ", "授乳ブラ", "ノンカフェイン", "その他"] },
  { name: "ギフトセット", id: "205222", keyword: "出産祝い ギフト", subs: ["ロンパース・服", "おもちゃ", "スキンケア", "タオル・スタイ", "食器・哺乳瓶", "ブランドギフト", "その他"] },
  // レンタル: 単一ジャンルに集約されていないため genreId は持たせず、
  // keyword + サブカテゴリのキーワードマッチで関連性を担保する
  { name: "レンタル", keyword: "ベビー用品 レンタル", subs: ["ベビーカー", "チャイルドシート", "ベビーベッド", "抱っこ紐", "おもちゃ", "その他"] },
];
...
export const parseQuantity = (name) => {
  ...
  if (m) return { count: parseInt(m[1], 10) * parseInt(m[2], 10), unit: '枚' };
  // 「A枚入り B個セット」など、×記号を使わないセット表記。
  // 「200枚入り 2個セット」を200枚と誤読していたため追加（実データでは
  // 「12個セット」「6個セット」等の大口セットも同様に取りこぼしていた）。
  // 間に数字を挟む場合（「52枚 6-11kg 4個セット」等）は対応関係が不確実なので
  // [^\d]{0,8} で数字をまたがせず、確実なケースだけを掛け算する。
  m = n.match(/(\d+)\s*枚[^\d×xX＊*]{0,8}?(\d+)\s*(?:個|パック|袋|セット|箱)/);
  if (m) return { count: parseInt(m[1], 10) * parseInt(m[2], 10), unit: '枚' };
  // 単純な「A枚」
  m = n.match(/(\d+)\s*枚/);
