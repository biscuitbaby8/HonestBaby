...
function extractSubCategory(category, itemName) {
  const rules = {
    ...
    "おもちゃ": [
      ...
      // 月齢表記なし → 製品タイプで判定
      { match: /メリー|ベッドメリー|モビール|ガラガラ|にぎにぎ|おくるみ/, sub: "0ヶ月〜" },
      { match: /歯固め|フィンガーパペット|ガスガス/, sub: "3ヶ月〜" },
      { match: /プレイマット|お座り|引き車|ソフトブロック|積み木|くもん/, sub: "6ヶ月〜" },
      { match: /乗用|ままごと|パズル|型はめ|ブロックセット|シルバニア|木製玩具|おままごと/, sub: "1歳〜" },
    ],
    "ベビーカー": [
      // 周辺グッズを先に判定（本体より優先）
